    ...
    <main className="min-h-screen bg-stone-50 text-stone-900">
      ...
      <section className="bg-white py-16">
        <div className="mx-auto max-w-4xl px-6">
          ...
          <div className="grid gap-8 sm:grid-cols-3">
            {[
              { step: "1", title: "Elige tu pack", desc: "Selecciona el pack o suscripción que prefieras." },
              { step: "2", title: "Paga seguro", desc: "Pago con Stripe. Recibimos tu pedido al instante." },
              { step: "3", title: "Recibe mañana", desc: "Entrega gratis en Santa Cruz / La Laguna." },
            ].map((s) => (
      ...
      <section className="py-16">
        <div className="mx-auto max-w-md px-6 text-center">
          ...
          <p className="mb-6 text-stone-500">Rastrea tu pedido con tu número de WhatsApp</p>
          <a
            href="/yogurt/orders"
...
function ProductCard({ product }: { product: any }) {
  return (
    <div className="flex flex-col rounded-2xl border border-stone-200 bg-white p-6 shadow-sm transition hover:shadow-md">
      ...
      <div className="mb-4 flex items-baseline gap-2">
        ...
        {product.comparePrice && (
          <span className="text-sm text-stone-400 line-through">€{product.comparePrice}</span>
        )}
      </div>
      ...
      <form action={createOrder}>
        ...
        >
          {product.isSubscription ? "Suscribirse" : "Comprar"}
        </button>
      </form>
    </div>
  );
